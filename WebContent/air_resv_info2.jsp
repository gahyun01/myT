...
<%
	...
	//	저장된 air_resv 테이블의 resv_no 값 가져오기
	AirResv resv = resvdb.selectRow( user_id, dept_no, dept_date, adult + child);
	int resv_no = 0, resv_no2 = 0;
	if(resv != null)
		resv_no = resv.getResv_no();
	
	if( trip.equals("2") ){
		resv = resvdb.selectRow( user_id, arrv_no, arrv_date, adult + child);
		if(resv != null)
			resv_no2 = resv.getResv_no();
	}
...
<body>
 ...
 <jsp:include page="banner.jsp" />
  <form action='air_resv_ins.myt?menu=air&gbn=3&trip=<%= trip %>' method='post'>
    <section>
	...
	<input type='hidden' class='hidden' name='hidden_resv_no' id='hidden_resv_no' value='<%= resv_no %>' > 
	<input type='hidden' class='hidden' name='hidden_resv_no2' id='hidden_resv_no2' value='<%= resv_no2 %>' > 
...
%>
      <div class="person_head">가는 일정<label style='margin-left: 880px; font-weight: normal; color: red'>출발일 기준 만12세 생일부터 성인 요금이 적용됩니다. </label></div>
      <div style="margin-top: 10px">
        <table border="0" cellspacing="0">
<%
			String date = dept_date.substring(4, 6) + "월 " + dept_date.substring(6, 8) + "일";
%>
          <tr>
            ...
            <td class="cont_resv" id='td_amount'><label class="money"><%= String.format("%,d", data.getPrice()) %></label> 원</td>
            <td class="cont_resv" ><label id='dept_person' ></label></td>
            <td class="cont_resv"><label class="money"  id='dept_amount'></label> 원</td>
...
<%		
	...
	if(trip.equals("2")){	//	왕복일 경우 오는 여정도 셋팅
		...
		if(data != null){
			String date = arrv_date.substring(4, 6) + "월 " + arrv_date.substring(6, 8) + "일";
%>
       ...
       <div style="margin-top: 10px">
        <table border="0" cellspacing="0">
          <tr>
            ...
            <td class="cont_resv"><%= data.getHour() %></td>
            <td class="cont_resv2"><%= data.getLevel() %></td>
            <td class="cont_resv" id='td_amount2'><label class="money"><%= String.format("%,d", data.getPrice()) %></label> 원</td>
            <td class="cont_resv" ><label id='arrv_person' ></label></td>
            <td class="cont_resv"><label class="money"  id='arrv_amount'></label> 원</td>
            <td class="cont_resv"  style='display: none;' ><%= dept_no %></td>
...
%>      
    <!--div class="line" style='margin-top: 20px'></div-->
      <div class="div_btn" style='margin-top: 20px'>
        <button type="button" class="btn btn_air_list"  style='background-color: blue; border: none' id="btn_resv" >최종 예약 확정</button>
        <div class="div_amount"><div class="div_amount1">최종 결제금액</div>
         <div class="div_amount2" id="total_amount"></div><div class="div_amount3">원</div></div>
      </div>
    </div><!-- bg_air -->

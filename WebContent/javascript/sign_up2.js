...
function checkAll(){
	...
	//	아이디 중복확인이 되었는지 hidden_input으로 체크
	str = tag[19].value;
	if(str == "1"){	//	아이디 중복의 경우
		ok = false;
	...
	//	비밀번호 확인이 되었는지 hidden_input으로 체크
	str = tag[20].value;
	console.log("tag[20].value 비번확인 : " + tag[20].value);
	if(str == "0"){	//	확인이 안되었을 경우
		ok = false;
		alert("비밀번호 동일확인을 해주세요.");
		return ok;
	}
...
function checkPasswd(btn){
	...
	var hid_pwd = document.getElementsByName('hid_pwd');
	console.log("before hid_pwd[0]:" + hid_pwd[0].value);
	...
	console.log("final hid_pwd[0]:" + hid_pwd[0].value);
}
...
$(document).ready(function(){
	...
	$('#user_id').focus(function(){
		var dupe = document.getElementsByName('hid_dupe');
		dupe[0].value = "1";
		console.log("아이디에 포커스가 감:" + dupe[0].value);
	});
	...
	$('#pwd, #pwd2').focus(function(){
		var hid_pwd = document.getElementsByName('hid_pwd');
		hid_pwd[0].value = "0";
		console.log("비번에 포커스가 감:" + hid_pwd[0].value);
	});


	//	아이디 중복체크 버튼
	$('#btn_id').click(function(){
		var nm = document.getElementsByName('user_id');
		user_id = nm[0].value;
		len = nm[0].value.length;

		if(len == 0 || len < 6 || len > 15 ){
			alert("6~15자리의 아이디로 입력하세요");
			nm[0].focus();
			nm[0].select();
			return;
		}
		...
		$.ajax({ // key value 형식
			url : "check_dupe.jsp?user_id=" + user_id,
			success : function(re){
				...
				if(tmp == "0"){	//	중복아이디 없음
					...
					dupe[0].value = "1";
					msg.textContent = "※ 이미 사용중이므로 사용할 수 없는 아이디입니다.";
					alert("※ 이미 사용중이므로 사용할 수 없는 아이디입니다.");
					nm[0].focus();
				}
			}
		});

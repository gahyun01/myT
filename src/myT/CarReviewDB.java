...
//	+----------+-------------+------+-----+---------+----------------+
	...
	public ArrayList<CarReview> selectAll() {
		...
		try{
			connectDB();
//			ResultSet rs = stmt.executeQuery("SELECT RV_NO, CITY_NO, CITY_KNM, CAPACITY, TITLE, CONT, USER_ID, DATE_FORMAT(REG_DATE, '%Y-%m-%d') DATE FROM CAR_REVIEW where city_knm = '아무거나' ORDER BY RV_NO DESC;");
			ResultSet rs = stmt.executeQuery("SELECT RV_NO, CITY_NO, CITY_KNM, CAPACITY, TITLE, CONT, USER_ID, DATE_FORMAT(REG_DATE, '%Y-%m-%d') DATE FROM CAR_REVIEW ORDER BY RV_NO DESC;");
			...
			while(rs.next()){
				...
				review.setCityNo(rs.getString("city_no"));
	...
	public ArrayList<CarReview> selectRow(String city, int capacity, String searchId, String searchText) {
		...
		ArrayList<CarReview> arry = null;
		String sql = "SELECT RV_NO, CITY_NO, CITY_KNM, CAPACITY, TITLE, CONT, USER_ID, DATE_FORMAT(REG_DATE, '%Y-%m-%d') DATE FROM CAR_REVIEW";
...
//			sql += " AND USER_ID = '" + searchId + "'";
	...
	public int insertRow(CarReview review) {
		String sql = "INSERT INTO CAR_REVIEW VALUES( NULL, NULL, '";
		int cnt = 0;
...
//		String sql = "UPDATE CAR_REVIEW SET CITY_KNM = '서울' WHERE RV_NO = 33; = " + rvNO + ";";
//		int cnt = 0;
//		
//		try{
//			connectDB();

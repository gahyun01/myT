	...
	public void PostDBinput(PostGetSet pgs){
		...
		try{
			...
			conn = DriverManager.getConnection(
				"jdbc:mysql://localhost:3306/planner?characterEncoding=utf8&useSSL=false", "root", "1234");
			if (conn == null) {
				throw new Exception("mydb 데이터베이스로 연결할 수 없습니다<br>");
			}
			stmt = conn.createStatement();
			
			int row = stmt.executeUpdate("INSERT INTO festival_review(rv_no, user_id, fest_title, fest_rcont) VALUES (DEFAULT, '"+pgs.getUser_id()+"', '"+pgs.getFest_title()+"', '"+pgs.getFest_rcont()+"');");
